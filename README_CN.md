...
以下裁剪图来自同一条已评分延时序列，并聚焦有云区域。左下角标签是 SunsetScore 写入评分缓存的实际分数。本次评分没有 5 分样本，因此使用 1、3、4 分分别展示低、中、高分典型案例。

| 低分 | 中分 | 高分 |
|:---:|:---:|:---:|
| ![低分示例：没有霞光着色的普通白灰云，评分为 1/5](docs/images/score-examples/low-score-1.jpg) | ![中分示例：着色清晰但较柔和的云层，评分为 3/5](docs/images/score-examples/medium-score-3.jpg) | ![高分示例：大范围云层呈现鲜艳着色，评分为 4/5](docs/images/score-examples/high-score-4.jpg) |
| **1 / 5**<br>普通白灰云，没有霞光着色。 | **3 / 5**<br>云层着色清晰，但仍较柔和、强度中等。 | **4 / 5**<br>大范围云层呈现鲜艳着色。 |
...
固定提示词要求模型选择一个互斥视觉类别，程序再映射为以下代表分数：

- `0`：没有可见天空，或完全没有火烧云证据
- `1`：没有可辨认的自然云层，或云体没有霞光着色，包括纯日落、暖色地平线和普通白云
- `2`：云体出现微弱、局部或难以确认的暖色或紫色
- `3`：云体着色清楚，但颜色较柔和且只占局部或较小范围
- `4`：云体颜色明显鲜艳或对比强烈，或者着色云层覆盖较广、横跨天空较大区域
- `5`：鲜艳强烈的着色云覆盖大部分可见天空，同时具有丰富层次且毫无歧义

离散类别避免小型生成式视觉语言模型给出的数字与其理由相互矛盾。建议仍把分数视为粗粒度信号，并在用于自动决策前使用自己的照片验证判定规则。

## 晚霞判定

分数大于等于 `3` 的采样照片属于高分照片。高分要求照片中存在可辨认且被红、橙、粉、金或霞光紫色染亮的云体；只有太阳、暖色天空或地平线的纯日落不能达到该阈值。采样位置不少于三个时，只要任意连续三个位置中至少存在两张高分照片，SunsetScore 就返回 `has_sunset = true`。该规则能够排除孤立高分，同时不会让长序列中短暂出现的晚霞被全片平均分稀释。采样位置不足三个时，任意一张高分照片都会产生肯定结果。
