...
- `5`：没有可见天空，或完全没有火烧云证据
- `18`：没有可辨认的自然云层，或云体没有霞光着色，包括纯日落、暖色地平线和普通白云
- `42`：云体出现微弱、局部或难以确认的暖色或紫色
- `62`：一个或多个云块的云体清楚呈现霞光颜色，但强度或覆盖范围有限
- `84`：火烧云明显而强烈，具有丰富纹理或较大覆盖范围
- `98`：大面积、强烈、层次丰富且毫无歧义的火烧云

离散类别避免小型生成式视觉语言模型给出的数字与其理由相互矛盾。建议仍把分数视为粗粒度信号，并在用于自动决策前使用自己的照片验证判定规则。

## 晚霞判定

分数大于等于 `50` 的采样照片属于高分照片。高分要求照片中存在可辨认且被红、橙、粉、金或霞光紫色染亮的云体；只有太阳、暖色天空或地平线的纯日落不能达到该阈值。采样位置不少于三个时，只要任意连续三个位置中至少存在两张高分照片，SunsetScore 就返回 `has_sunset = true`。该规则能够排除孤立高分，同时不会让长序列中短暂出现的晚霞被全片平均分稀释。采样位置不足三个时，任意一张高分照片都会产生肯定结果。
